...
def all_users():
    # users_no_ID = map(lambda user: ,users) 
    users_no_id = {user_name: {"name": user_data["name"], "description": user_data["description"]} for user_name, user_data in sorted(users.items())}
    return pretty_json(users_no_id)
